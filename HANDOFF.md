...
**Commits locais ainda não pushed/deployados (`38ec9bd`, `a10bf76`, `f7c26d2`, `9de3b47`; 29/07):** higiene do standalone, fail-fast do deploy, idempotência do pipeline pago, CI, integridade do job store e gate financeiro. `.tmp` foi excluído do file tracing; o artefato caiu de 713 MB para 90 MB no disco e ganhou `npm run verify:standalone` (exige `server.js`, rejeita `.tmp` e tamanho acima de 200 MB). O rsync preserva o `.tmp` runtime da VPS. `deploy_studio.sh` agora aborta em qualquer falha, preserva o env remoto por padrão, só o substitui com `DEPLOY_ENV_FILE=/caminho`, reinicia o PM2 com `--update-env` e exige health check local HTTP 200/401. `POST /api/pipeline/run` exige `Idempotency-Key` e aprovação explícita com até 10 minutos: concorrência/replay com a mesma chave+payload reutiliza o job; chave igual com payload diferente retorna 409; aprovação ausente/expirada retorna 403 antes de persistência/provedores. A UI pede confirmação humana (~US$3–6), mantém chave+aprovação em retries e possui lock síncrono contra double-click. Estado de jobs é gravado atomicamente (`temp → rename`) e IDs em status/download/delete exigem UUID, bloqueando traversal e injeção de filtro. O VOICE_GATE checa configuração antes de persistir, evitando episódio fantasma quando a chave ElevenLabs está ausente. `.github/workflows/quality.yml` executa typecheck, contrato do deploy, build, verificação standalone e idempotência autocontida sem secrets. A mesma sequência passou localmente. O lint global continua vermelho por débitos preexistentes.
